-- Supabase port of netlify/database/migrations/0001_initial_schema.sql
--
-- 1:1 schema port from the Netlify+Neon stack. Custom-auth `users` table is
-- kept for now; a later migration in this branch introduces a `profiles`
-- table linked to Supabase Auth (`auth.users`) and the 19 leadership rows
-- get migrated across via magic-link force-reset before the cutover.
--
-- Trigger functions and DO blocks are Postgres-portable — they run on
-- Supabase unchanged.
...
-- Committee head/vice-head FKs — deferred because members didn't exist when
-- committees was created above.
...
-- ─── USERS (custom auth, will be migrated to auth.users later) ──────────
...
-- ─── PROJECTS / EVENTS (combined entity) ────────────────────────────────
